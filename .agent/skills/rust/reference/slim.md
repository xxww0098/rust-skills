...
目的：用数据把构建时间和 Rust 开发反馈成本打下来。构建性能铁律：**无 `--timings` 不改性能配置**（BUILD-04/META-02）；冷构建与热增量分开诊治（BUILD-03）。磁盘被 `target/`、过期产物和孤儿源文件吃满时走 [hygiene.md](slim/hygiene.md)，**禁止用 `cargo clean` 当加速**（BUILD-07/11）。`/cargo tools` 可以按仓库事实做工具盘点、命令去重、版本/安装源归一和孤儿配置清理，但只要声称“更快”仍必须给同指纹前后基线。

编排：多文件时按 [kernel/swarm.md](../kernel/swarm.md) — 四条 pin 即四条车道：cargo 指纹 · test 证明集 · tooling owner · hygiene 四层。禁止各跑一遍 metadata。`--timings` 与复测必须串行、同一 target-dir。卫生车道禁止清共享 target。 单文件或已有快照则跳过。
...
- `/cargo hygiene [target]` → [过期开发文件分层清理](slim/hygiene.md)：`target/`、`$CARGO_HOME`、未入库垃圾、入库孤儿四层分开扫；`cargo clean` 不是加速。
- `/test [target]` → [最小充分测试](slim/test.md)：深读改动面、测试目标和不变量，只编译并运行足以证明本次改动的测试，合并重复证明。

同一请求同时涉及 build、tools、test 与 hygiene 时，本轮只消费一份 ProjectSnapshot：`/cargo` 固定 toolchain/target/profile/features/target-dir 指纹，`/cargo tools` 对齐编辑器/终端/CI 与该指纹，`/test` 在同一图上选最小证明集，`/cargo hygiene` 只读这份图的 `orphans` 与体积证据。禁止各 playbook 各跑一遍 metadata、各画一遍 crate 图、各维护一套 features，或切换 target dir 造成伪冷构建。禁止用清缓存冒充构建收益。

## 步骤

0. **先分诉求**：构建慢/链接慢/重编译 → `slim/cargo.md`；工具太多、IDE/终端重复、装工具慢、新人难启动 → `slim/tooling.md`；测试慢/重复跑 → `slim/test.md`；磁盘涨、过期开发文件、孤儿 `.rs`、误提交的火焰图/`perf.data` → `slim/hygiene.md`。同一根因只由一个 owner 报告。
...
无 target /“编译太慢”类诉求：从 ProjectSnapshot 的 workspace default-members 与当前改动包推导最可能主产物；只有出现两个同等主路径且代价差异巨大时才问一次，禁止先跑整仓。无 target /“开发体验差”类诉求：先体检权威 bootstrap、编辑环和提交前入口，不先推荐工具名单。无 target /“磁盘/过期文件”类诉求：先出四层体积表，不先 `cargo clean`。
...
- **同时走 cargo/tools/test/hygiene**：额外给“避免的重复工作”一栏，列出未再执行的 workspace、target、feature、测试二进制、watcher、安装步骤、同义门禁，以及未清的共享缓存。
