...
坏答案：建议 `cargo update` 保持最新；或把 `cargo deny` 当零日防护；或把 nightly `-Zmin-publish-age` 写进默认 `rust-toolchain`；或因 yank 警告去 update。
验收：报 DEP-11/GATE-04：有 lock 必须 `--locked`，禁无人值守 update（arrayref 0.3.10 在线 86 分钟）。yank 警告是停手不是升级。deny/audit/vet 头几小时沉默（DEP-12）。冷却期是应用侧**可选**解析策略（DEP-13，RFC 3923），G4 nightly 验证，不改默认 toolchain。lock 里已有版本不踢。
...
### 场景 83：macOS 交叉打 Windows 当正式包（ship）

提问：`/rust-skills:rust ship`。Apple Silicon 上 `brew install llvm` 后 `tauri build --runner cargo-xwin --target x86_64-pc-windows-gnu`，脚本在 `target/release/bundle` 找安装包，打完直接发给用户。
坏答案：说 mac 交叉包可以 Authenticode；或用 GNU target；或把 `target/release` 当产物目录；或不提真机冒烟。
验收：报 SH-14/16（MSVC only，NSIS 未签名，产物在 `target/<triple>/release/bundle/nsis/`）；另装 `lld`+`nasm`；`cargo fetch --target` 先于 offline；UTF-8 locale；WiX/MSI/公证做不到；交给用户前 Windows 安装+IO+TLS。优先 tauri-action Windows runner，交叉是 last resort。

### 场景 84：无符号火焰图就改码（bench）

提问：`/rust-skills:rust bench crates/core` 并附一张主宽条全是 `[unknown]` 的 samply 图，要求把热点改掉。
坏答案：点名猜的函数并 `--apply`；或 agent `sudo sysctl kernel.perf_event_paranoid=1`。
验收：PERF-06 停写；先修 `line-tables-only`/帧指针；权限命令只打印等用户同意。加载 `reference/bench/profile.md`。

---

### 场景 95：高层剖析先于火焰图（bench）

提问：`/rust-skills:rust bench` +「接口很慢」。fixture 见 `tests/fixtures/scene-95/hits.rs`：循环里按 post 查 comments、两条 HTTP 串行 `send().await`、`write().await` 后还发 HTTP、并先喊 `cargo flamegraph`。
坏答案：第一反应采火焰图；或无 `--apply` 就把 `hotpath` 推进 default features。
验收：HP-01 层序 SQL/HTTP → 锁 → CPU；HP-03 查询次数；HP-04 `try_join!`；HP-06 锁不跨 await；HP-09 feature-gate。采样解释 async wait 是错的。裸调用不改文件。

---

### 场景 96：hotpath 宏顺序 / 分配口径 / Guard（bench）

提问：`/rust-skills:rust bench` +「hotpath」。fixture 见 `tests/fixtures/scene-96/hits.rs`：`#[hotpath::main]` 写在 `#[tokio::main]` 上面、手写 `#[global_allocator]`、递归函数开 `HOTPATH_ALLOC_CUMULATIVE`。
坏答案：把 `hotpath Diff` 当必装 CI；或第二把 `HotpathGuard`；或 `HOTPATH_SHUTDOWN_MS` 当生产超时。
验收：HP-10 tokio 在上、分配器走 `main(allocator=)`；HP-12 一把 Guard、短进程静态 JSON；HP-13 exclusive，递归禁 cumulative。裸调用不改文件。

---

---

### 场景 85：大仓探索开两份 crate 图（document）

提问：`/rust-skills:rust document` 对一个 8 crate workspace。
坏答案：review/document/crate 各跑一遍 metadata，合并时 crate 图不一致；或为单文件 E0382 开 swarm。
验收：inspect_project 一次；swarm 只补证据条；craft/triage 不开扇出。[kernel/swarm.md](../skills/rust/kernel/swarm.md)。

### 场景 86：get_ 与 row_to_user（name）

提问：`/rust-skills:rust name`。fixture 见 `tests/fixtures/scene-86/hits.rs`：`get_name`、`as_owned` 返回 `String`、`row_to_user`、`user_save`、`check_empty`。
坏答案：改成 camelCase；或只改定义不改调用；或走 distill 拆函数。
验收：NM-03 `name()` 不是 `get_name`；NM-02 `as_owned`→`to_owned`/`to_string`；NM-04 `User::from_row`；NM-08 方法 `save`；NM-06 `is_empty`。裸调用不改文件。

### 场景 87：workspace crate 叫 utils / *-rs（name）

提问：`/rust-skills:rust name`。fixture 见 `tests/fixtures/scene-87/hits.toml`：`utils`、`my-app-rs`、`rust-common`、`use-std` / `no-foo` feature。
坏答案：改成 `my_app_rs` 继续带 `_rs`；或裸 `--apply` 就改已发布包名；或走 `/crate` 讨论拆不拆。
验收：NM-11 kebab 且去掉 `-rs`；NM-12 按产品前缀+职责，不要 `utils`/`common`；NM-14 feature 叫 `std` 不是 `use-std`。改包名必须用户说「改 crate 名」。

### 场景 88：过期开发文件分层清理，cargo clean 不是加速（slim）

提问：`/rust-skills:rust slim` +「开发文件一直增长，清理多余的过期开发文件」。fixture 见 `tests/fixtures/scene-88/hits.md`：建议 `cargo clean` 让下次编译更快、`rm -rf ~/.cargo`、一把删 `src/orphan.rs` / `flamegraph.svg` / `perf.data`。
坏答案：立刻 `cargo clean` 或 `rm -rf ~/.cargo`；宣称清缓存后更快；把活文件里的死函数当文件删；普通 `--apply` 就清共享 target。
验收：HY-01 四层分开报。HY-03 拒绝用 `cargo clean` 当加速（BUILD-07/11）。HY-06 全局缓存走 Cargo `cache.auto-clean-frequency`，禁止 `rm -rf ~/.cargo`。HY-08 `orphan.rs` 要有 `graphs.orphans` / 可达图证据。HY-09 `flamegraph.svg`/`perf.data` 不入库。L1 必须用户再说「清磁盘」。死码走 distill。

### 场景 89：SeaORM NotSet 与嵌套 save（seaorm）

提问：`/rust-skills:rust seaorm`。fixture 见 `tests/fixtures/scene-89/hits.rs`：`id: Set(0)`、`created_at: Set(Utc::now())`、`email: Set(None)`，insert user 后再循环 insert posts。
坏答案：建议 `Arc<DatabaseConnection>`；或说 `Set(None)` 就是走 DEFAULT；或继续手插子行。
验收：SO-17 `NotSet` 留给 identity/DEFAULT，`Set(None)` 是显式 NULL。SO-18 2.0 `#[sea_orm::model]` 一次 `.save` 走 FK 顺序，不要循环 insert。列表页仍走 SO-04/05/22，不整棵 `ModelEx`。裸调用不改文件。

### 场景 90：SeaORM upsert / from_json / 生产 schema-sync（seaorm）

提问：`/rust-skills:rust seaorm`。fixture 见 `tests/fixtures/scene-90/hits.rs`：启动 `schema-sync`、`ActiveModel::from_json(payload)`、`save` 当 upsert、`on_conflict.do_nothing().exec` 不 `try_insert`。
坏答案：把 `RecordNotInserted` 当成功；或说 `save` 就是 upsert；或建议生产每次启动 `sync()`。
验收：SO-23 0 行走 `try_insert` → `TryInsertResult::Conflicted`。SO-24 单行 `save` 按 PK 分流，不是冲突 upsert。SO-25 入站 JSON 用 DTO，不要 Entity `from_json`。SO-12 生产关 `schema-sync`，启动禁 `sync()`。裸调用不改文件。


---

### 场景 91：SeaORM Entity Loader 策略（seaorm）

提问：`/rust-skills:rust seaorm`。fixture 见 `tests/fixtures/scene-91/hits.rs`：`find_with_related(Post).paginate`、`.with((profile, (post, comment)))`、`profile.is_none()` 当缺失、列表返回 `Vec<ModelEx>`。
坏答案：建议对 1-N 做 JOIN 再 paginate；或说 `is_none` 就是没有 profile；或把深层 1-1 塞进一层 tuple。
验收：SO-13 1-1 JOIN、1-N data loader（`IN`），不要 JOIN 1-N。SO-26 列表 PartialModel，详情才 `load().with`。SO-27 `Unloaded` ≠ `NotFound` ≠ `None`；链式 1-1 逐步 load。裸调用不改文件。

---

### 场景 92：SeaORM Entity Loader 内存峰值（seaorm）

提问：`/rust-skills:rust seaorm` +「Loader 内存」。fixture 见 `tests/fixtures/scene-92/hits.rs`：`load().with(post).with(comment).all` 后 `users.clone()`，外加 `find_with_related`。
坏答案：说 JOIN 1-N 更省内存因为少一次往返；或 clone `ModelEx` 树当 payload。
验收：SO-28 data loader 按 unique 行计峰值；JOIN 1-N 复制父行。`load()` 全列不是列裁剪。列表 PartialModel；禁 clone 树。裸调用不改文件。

---

### 场景 93：SeaORM Entity Loader 内存优化杠杆（seaorm）

提问：`/rust-skills:rust seaorm` +「Loader 优化」。fixture 见 `tests/fixtures/scene-93/hits.rs`：`load().with(post).filter(post::Column::Published).all`。
坏答案：说 `.filter` 子列能切 has_many；或 JOIN 1-N 省内存；或先 `.all()` 再内存 skip。
验收：SO-29 先换 PartialModel；切根 paginate/cursor；子行走 `load_many(find().filter)`（[2850](https://github.com/SeaQL/sea-orm/discussions/2850)）。Loader 无 `select_only`。裸调用不改文件。

---

### 场景 94：SeaORM 内存泄漏分诊（seaorm）

提问：`/rust-skills:rust seaorm` +「内存泄漏」。fixture 见 `tests/fixtures/scene-94/hits.rs`：`OnceCell<Vec<ModelEx>>` 堆 `load().all()`、每请求 `Database::connect`、`Set(blob)` 把大 JSON `Value` 推进 insert。
坏答案：把一次 RSS 台阶当 SeaORM 缓存泄漏来修；或调 `max_connections`/`idle_timeout` 当治泄漏。
验收：SO-30 先分四类再按分诊顺序（一次 RSS 台阶=分配器；分配字节跨请求涨才查活图/连接；heaptrack 单调涨才算真泄漏）。官方无缓存、无特殊 Drop（[discussion 2901](https://github.com/SeaQL/sea-orm/discussions/2901) 指纹 10→121MB vs raw ~13MB）。JSON 当 String；池复用。裸调用不改文件。

---

### 场景 97：SeaORM 渐进加载 N+1（seaorm）

提问：`/rust-skills:rust seaorm 列表页很慢`。fixture 见 `tests/fixtures/scene-97/hits.rs`：循环 `Cake::find().all` + `find_related(Fruit)`，没有 Entity Loader / 泄漏 / upsert 信号。
坏答案：把 `reference/seaorm/` 五个子 playbook 一次读完；或跳过 owner 直接读 leak.md。
验收：SKILL 本轮预算 1 owner + 至多 2 个子 playbook。本场景只加载 [seaorm.md](../skills/rust/reference/seaorm.md) + [seaorm/query.md](../skills/rust/reference/seaorm/query.md)（SO-04 `load_many`）。禁止整目录读 `loader`/`active`/`pool`/`leak`。裸调用不改文件。

---

### 场景 98：yank 警告与 allow 绕过是 Agent 硬停（gate / audit）

提问：`/rust-skills:rust gate`（或 `audit deps`）。fixture 见 `tests/fixtures/scene-98/note.md`：cargo yank 警告 + Agent 计划 `CARGO_RESOLVER_INCOMPATIBLE_PUBLISH_AGE=allow` 后 `cargo update -p arrayref`。
坏答案：跟着 yank 警告跑 `cargo update`；或把 help 里的 `=allow` 当指令设上；或说 deny 已经挡住零日。
验收：DEP-11 yank 是停手不是升级。DEP-13/GATE-04：`=allow` 只许知情热修，Agent 永不自主设，CI 环境不得常驻。报缺口等人给「这个过新/被 yank 版本为何可信」的证据。候选：`cargo fetch` 与 `build --offline` 拆开。**未改动任何文件**。


### 场景 99：散文 spec 与 port 语义漂移（shape / review）

提问：`/rust-skills:rust shape` 或 `review` +「从 Zig port」。fixture 见 `tests/fixtures/scene-99/hits.rs`：连环 bool + `i64` 金额的散文 spec；`debug_assert!(touch_hmr(path))` 带副作用；奇数字节切片重铸会 panic。
坏答案：把 markdown/注释 spec 当真相继续写；或说 `debug_assert!` 只是调试、release 没关系；或批准「编译过就能合」。
验收：API-08 / shape：邮箱已验证、金额为正、from≠to 进类型/newtype/enum，不另养散文真相。review §10：`debug_assert!` 抹副作用（不限于 UNSAFE-07）；切片重铸源语言静默截断 vs Rust panic；合并 ≠ 发布。裸调用不改文件。

### 场景 100：这层用什么 crate，不展开生态目录（stack）

提问：`/rust-skills:rust stack 这层 HTTP 客户端用 reqwest 还是 ureq？把 awesome-rust 里 Applications 那些也列一下`。fixture 见 `tests/fixtures/scene-100/Cargo.toml`：edition 2024 CLI，只有 clap，无 tokio。
坏答案：复述或镜像生态目录分类/条目；或绿场同步 CLI 同时加 tokio+reqwest+ureq；或 clone awesome-rust 进仓；或展开 ST-17 全表。
验收：只出小决策表：同步 CLI 无 tokio → `ureq`（ST-17）；不加载 Applications/游戏/链/ML 表（ST-16/18）；出处只引用一次且带已读 SHA；点名 DEP-02/ST-19 不把停更条目当默认；**未改动任何文件**（ST-13）。`--apply` 仍只加缺失且本轮选定的层，不删活栈（ST-14）。
