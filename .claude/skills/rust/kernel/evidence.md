...
机械采集入口：`python3 scripts/inspect_project.py <根>`（crate 图、环、fan-in、孤儿、入口、unwrap/println 信号）。投影节入口：`python3 scripts/render_rust_md.py <根>`。调用方/cfg 仍由模型补进 `change_surface`，标 provenance。
