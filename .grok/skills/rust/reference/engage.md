...
| 「SeaORM / ActiveModel / N+1 / Entity Loader / ModelEx / Unloaded / Loader 内存 / Loader 优化 / over-fetch / 嵌套保存 / NotSet / upsert / from_json / schema-sync」 | 加载 [seaorm.md](seaorm.md)，只体检 | 不把 Model/`ModelEx` 当 API；不 clone `ModelEx` 树；不把 Unloaded 当 None；`with().filter(子列)` 不切 has_many；不把 `save` 当 upsert；不双开 sqlx |
