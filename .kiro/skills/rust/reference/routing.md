...
按 [SKILL 输出契约](../SKILL.md) 组织：一句话结论 → 范围行 → 正文 → 验证 → 置信度 → 下一步 → 写授权收尾。 先给 2–3 个带理由的推荐（每个给出可直接复制的完整命令，如 `/rust-skills:rust review crates/storage`），然后是按分类分组的完整命令表（路由表「分类」列即分组依据）；以「想执行哪条？回复命令即可」收尾。**只推荐，不执行。**
